...
namespace SistemaGestionCotizaciones.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            var str = string.Join(";", this.ControllerContext.RouteData.Values.Select(x => x.Key + ":" + x.Value).ToArray());
            logger.Debug(str);
        }
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    logger.Info(string.Format("Login [{0}]",model.Username));
                    return RedirectToLocal(returnUrl);
                case SignInStatus.LockedOut:
                case SignInStatus.RequiresVerification:
                    logger.Info(string.Format("Login [{0}]", model.Username));
                    ModelState.AddModelError("", "No está registrado en el sistema SGC");
                    return View(model);
                case SignInStatus.Failure:
                default:
                    logger.Info(string.Format("Invalid Login [{0}]",model.Username));
